...
> :point_up: **Note!** Any code, application, or software that can perform an HTTP request can be a client.
...
The client requests hypermedia through a **Uniform Resource Locator (URL)**. The URL is the address of a unique hypermedia resource on the web.
...
URLs that contain the scheme and authority are considered to be **absolute URLs**. URLs that do not contain the scheme and authority are considered to be **relative URLs**, as in relative to the location from which the request with the relative URL was performed.
