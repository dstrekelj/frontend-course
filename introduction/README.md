# Frontend Course / Introduction

[:arrow_backward: Frontend Course](../README.md)

---

## Table of Contents

- [How To Use This Course](./00.md)
- [Setting Up](./01.md)
  - [Visual Studio Code](./01.md#visual-studio-code)
  - [Git](./01.md#git)
    - [GitHub](./01.md#github)
      - [SSH authentication](./01.md#ssh-authentication)
    - [GitHub Desktop](./01.md#github-desktop)
  - [Web Browsers](./01.md#web-browsers)
  - [Node.js and NPM](./01.md#nodejs-and-npm)
- [The Internet and the Web](./02.md)
  - [Internet Protocol](./02.md#the-internet-protocol)
  - [TCP and UDP protocols](./02.md#tcp-and-udp-protocols)
    - [Transport Control Protocol (TCP)](./02.md#transport-control-protocol-tcp)
    - [User Datagram Protocol (UDP)](./02.md#user-datagram-protocol-udp)
  - [HTTP protocol](./02.md#http-protocol)
    - [HTTP request message](./02.md#http-request-message)
    - [HTTP response message](./02.md#http-response-message)
    - [HTTP request methods](./02.md#http-request-methods)
    - [HTTP response status codes](./02.md#http-response-status-codes)
  - [The World Wide Web](./02.md#the-world-wide-web)
    - [Domains](./02.md#domains)
    - [URL](./02.md#url)
    - [The evolution of the Web](./02.md#the-evolution-of-the-web)

---

[:arrow_backward: Frontend Course](../README.md)
